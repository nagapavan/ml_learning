...
model_path = os.path.join(SCRIPT_DIR, "best_model_svm.pkl")
...
@st.cache_data
def load_eval():
    eval_path = os.path.join(SCRIPT_DIR, "eval_artifacts.pkl")
    try:
...
bean_info = {
    "BARBUNYA": {
        "emoji": "🟤",
        "desc": "Medium-large, kidney-shaped with a speckled pattern. Common in Turkish cuisine.",
    },
    "BOMBAY": {
        "emoji": "⚫",
        "desc": "The largest bean variety. Dark-coloured, smooth and oval-shaped.",
    },
    "CALI": {
        "emoji": "🟠",
        "desc": "Large bean with an oval shape and light colour. Also known as Calı.",
    },
    "DERMASON": {
        "emoji": "🟡",
        "desc": "Smallest bean variety. Round shape with a pale yellow colour.",
    },
    "HOROZ": {
        "emoji": "🔴",
        "desc": "Medium-large with a distinct elongated and slightly curved shape.",
    },
    "SEKER": {
        "emoji": "⚪",
        "desc": "Round, smooth and shiny white bean. 'Seker' means sugar in Turkish.",
    },
    "SIRA": {
        "emoji": "🟢",
        "desc": "Medium-sized with an oval shape. Very similar to Dermason but slightly larger.",
    },
}
...
with col1:
    area = st.number_input(
        "Area (px²)", min_value=20000, max_value=260000, value=53000, step=100
    )
...
with col2:
    ...
    convexarea = st.number_input(
        "Convex Area (px²)", min_value=20000, max_value=265000, value=53768, step=100
    )
    ...
    extent = st.number_input(
        "Extent", min_value=0.55, max_value=0.87, value=0.75, step=0.001, format="%.6f"
    )
...
predict_clicked = st.button(
    "🔍 Classify Bean", type="primary", use_container_width=True
)
...
if eval_data is None:
    ...
    with tab2:
        ...
        comp_df = comp_df.sort_values("Test Acc", ascending=False).reset_index(
            drop=True
        )
        ...
        st.dataframe(
            comp_df.style.highlight_max(
                subset=["Test Acc", "F1 Score"], color="#d4edda"
            )
    ...
    with tab3:
        ...
        st.caption(
            "Shows how many samples exist per bean class in the original training dataset."
        )
        ...
        dist_df["Percentage"] = (dist_df["Count"] / dist_df["Count"].sum() * 100).round(
            2
        )
        ...
        with d2:
            ...
            for i, (count, pct) in enumerate(
                zip(dist_df["Count"], dist_df["Percentage"])
            ):
